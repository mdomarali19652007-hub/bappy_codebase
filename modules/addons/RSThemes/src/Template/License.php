...
class License
{
    ...
    private static function loadRemoteLicense($licenseKey, $version, $templateName)
    {
        ...
        if (!empty($licenseKey)) {
            ...
            $results["registeredname"] = "profelar";
            $results["email"] = "profelar";
            $results["serviceid"] = "011";
            $results["productid"] = "20";
        ...
        if (0 < strlen($this->licenseDetails["nextduedate"]) && $this->licenseDetails["nextduedate"] != "0000-00-00") {
            ...
            if (0 <= $dueDateDiff && $dueDateDiff <= 14 && !$notShow) {
                ...
                if ($dueDateDiff == 0) {
                    ...
                    $html = str_replace("<b><span class=\"hidden\">Lagom WHMCS Theme -&nbsp;</span>", "<b>Lagom WHMCS Client Theme - ", $html);
                } else {
